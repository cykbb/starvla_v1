...
### Audit 结果(已完成 2026-05-26)

```
① GC flag wiring:                    ❌ NOT wired.
   yaml flag `trainer.enable_gradient_checkpointing: true` 出现在 4 个配置文件里,
   但 starVLA/ 全局 grep 找不到任何 model.gradient_checkpointing_enable() 调用。
   → Phase 3 必须做(不是 skip),这是当前训练慢的一大原因。

② extract_patch_features 调用点:    2 处,都在 QwenPaDTPI:
   - QwenPaDTPI.py:234 (forward / 训练路径)
   - QwenPaDTPI.py:324 (predict_action / eval 路径)
   _maybe_sampled_action_branch (line 187-225) 复用 forward() 传入的 patch_features,
   不单独调 extract_patch_features。
   → Phase 1 改 3 个调用点(包括 sampled branch 透传)。

③ outputs.logits 消费者:             仅 2 处,都流向 compute_dynamic_logits:
   - QWen2_5_PaDT.py:728 (forward_dynamic 训练路径)
   - QWen2_5_PaDT.py:928 (custom_vrt_decode AR 解码路径)
   没有其他消费者。
   → Phase 2 走方案 A(in-place 原位写)安全,无副作用。

④ model.visual 外部 hookers:          无。
   只有 2 处使用 model.visual:
   - _visual_forward_dual_res (line 281): 手动展开 visual 内部 (blocks/merger)
   - _replace_image_embeddings (line 455): 整体调用 self.model.visual(...)
   没有外部代码 hook 它的输出形状。
   → Phase 1 改 _replace_image_embeddings 签名安全。

⑤ PaDTObjectDecoder 下游消费者:      仅 condition_bridge。
   object_memory 流向 PaDTConditionBridge.forward (line 48, 83),
   object_view_memory 同样路径。
   → Phase 6 改 decoder 内部 hidden_dim 时,只要 object_memory_proj 出口
     仍输出 2048 给 condition_bridge,接口就稳定,无需改 bridge 代码。

⑥ tie_word_embeddings:                ✅ Qwen2.5-VL-3B 默认为 true。
   playground/Pretrained_models/Qwen2.5-VL-3B-Instruct/config.json: "tie_word_embeddings": true
   starVLA 代码本身没有显式 set/read 它。
   → Phase 7 极大简化:embed_tokens.weight 和 lm_head.weight 是同一个张量。
     去掉 VRT 行只需操作一份权重,无 sync 问题。

⑦ tokenizer.added_tokens 状态:        基础 + PaDTPI 扩展。
   - 基础(pretrained): 22 个 added tokens at IDs 151643-151664
     (Qwen 原生的特殊 token 如 <|endoftext|>、<|im_start|> 等)
   - PaDTPI 扩展(`QWen2_5_PaDT.py:120`): add_special_tokens 加 267 个
     = 5 控制 (padt_begin/end/reason_begin/end/null) + 4 obj + 2 view + 256 vrt
   - resize_token_embeddings 之后: vocab_size ≈ 151665 + 267 = 151932
   → Phase 7 改造:VRT 256 个从词表内删,控制 + obj + view 共 11 个保留。
     vocab_size 降回 ≈ 151676,embed_tokens / lm_head 砍掉 256 行(因为 tie 是同一张量,一次操作完成)。

补充发现:
- forward_dynamic / custom_vrt_decode 只在 QwenPaDTPI 内部调用,
  没有外部 framework 复用。Phase 1 改签名(加 precomputed_image_embeds)零外溢风险。
- resize_token_embeddings 只在 QWen2_5_PaDT.py:122 调用一次。
  另有 starVLA/model/modules/vlm/tools/add_qwen_special_tokens/ 是离线工具,
  不在训练/推理热路径上,Phase 7 可忽略。
```
