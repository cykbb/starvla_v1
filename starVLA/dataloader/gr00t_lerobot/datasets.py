...
from starVLA.dataloader.padt_segmentation_adapter import PaDTSegmentationSourceAdapter
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._output_image_size = self._resolve_output_image_size()
        self._persist_output_image_size()
        ...
        self._padt_segmentation_adapter = PaDTSegmentationSourceAdapter(
            dataset_path=self.dataset_path,
            tasks_df=self._tasks,
            data_cfg=self.data_cfg,
        )
        self._answers = self._get_answers()
    ...
    def answers(self) -> pd.DataFrame:
        """Conversation-based annotations keyed by task_index."""
    ...
    def _get_trajectories(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        # v3.0
            ...
            for file_path in file_paths:
                ...
                # Collect video-specific chunk/file index columns
                video_chunk_cols = {
                ...
                for index, episode in episodes_data.iterrows():
                    ...
                    # Store per-video-key chunk/file indices (may differ from data chunk/file)
                    video_chunk_indices = {}
                    video_file_indices = {}
                    for vkey, col in video_chunk_cols.items():
                        val = episode[col]
                        if not pd.isna(val):
                            video_chunk_indices[vkey] = int(val)
                    for vkey, col in video_file_cols.items():
                        val = episode[col]
                        if not pd.isna(val):
                            video_file_indices[vkey] = int(val)
                    ...
                    episode_meta = {
                        ...
                        "videos/chunk_indices": video_chunk_indices,
    ...
    @classmethod
    def _parse_image_size(cls, value) -> tuple[int, int] | None:
        if value is None:
            return None
        ...
        if isinstance(value, Sequence) and not isinstance(value, (str, bytes)):
            if len(value) >= 2:
                height = cls._to_int_scalar(value[-2])
                width = cls._to_int_scalar(value[-1])
                return (width, height)
            if len(value) == 1:
                edge = cls._to_int_scalar(value[0])
                return (edge, edge)
            return None
        edge = cls._to_int_scalar(value)
        return (edge, edge)

    def _resolve_output_image_size(self) -> tuple[int, int]:
        default_size = (224, 224)
        if self.data_cfg is None:
            return default_size

        image_size = self._parse_image_size(self.data_cfg.get("image_size", None))
        if image_size is not None:
            return image_size

        default_image_resolution = self._parse_image_size(
            self.data_cfg.get("default_image_resolution", None)
        )
        if default_image_resolution is not None:
            return default_image_resolution

        return default_size

    def _persist_output_image_size(self) -> None:
        if self.data_cfg is None:
            return
        width, height = self._output_image_size
        image_size = [width, height]
        try:
            self.data_cfg.image_size = image_size
        except Exception:
            self.data_cfg["image_size"] = image_size

    def _is_data_cfg_enabled(self, key: str, default: bool = False) -> bool:
        if self.data_cfg is None:
            return default
        value = self.data_cfg.get(key, default)
    ...
    def _get_spatial_merge_size(self) -> int:
        ...
        merge_size = self.data_cfg.get("spatial_merge_size", default_merge)
        return max(1, self._to_int_scalar(merge_size))
    ...
    def _get_answers(self) -> pd.DataFrame:
        """Get conversation-based annotations for the dataset."""
    ...
    def mask_to_patches(
        ...
        """Convert an RLE mask into merged patch indices."""
        import warnings
        ...
        try:
            ...
            warnings.warn(
                "pycocotools not installed. Segmentation patch extraction is disabled.",
                UserWarning,
            )
            return []
        ...
        grid_w = image_width // merged_patch_size
        grid_h = image_height // merged_patch_size
        ...
        row_idx = (np.arange(image_height) * mask_h / image_height).astype(int)
        col_idx = (np.arange(image_width) * mask_w / image_width).astype(int)
        row_idx = np.clip(row_idx, 0, mask_h - 1)
        col_idx = np.clip(col_idx, 0, mask_w - 1)
        resized = binary_mask[np.ix_(row_idx, col_idx)]

        patches = []
        for merged_row in range(grid_h):
            row_start = merged_row * merged_patch_size
            row_end = row_start + merged_patch_size
            for merged_col in range(grid_w):
                col_start = merged_col * merged_patch_size
                col_end = col_start + merged_patch_size
                region = resized[row_start:row_end, col_start:col_end]
                if region.any():
                    patches.append(merged_row * grid_w + merged_col)
        return patches

    def _enrich_seg_with_patches(self, seg_item: dict) -> dict:
        """Inject merge-grid patch indices computed from each segmentation mask."""
        if not isinstance(seg_item, dict):
            return seg_item

        seg_item = copy.deepcopy(seg_item)
        if not self._is_data_cfg_enabled("include_seg_patches", False):
            return seg_item

        spatial_merge_size = self._get_spatial_merge_size()
        for _, obj in seg_item.items():
            if not isinstance(obj, dict):
                continue
            mask_info = obj.get("mask")
            if not mask_info:
                continue
            obj["patches"] = self.mask_to_patches(
                mask_info,
                image_size=self._output_image_size,
                spatial_merge_size=spatial_merge_size,
            )
        return seg_item

    def _get_task_index_for_step(self, step_index: int) -> int | None:
        if self.curr_traj_data is None:
            return None
        if "task_index" in self.curr_traj_data.columns:
            return self._to_int_scalar(self.curr_traj_data["task_index"].iloc[step_index])

        lang_keys = self.modality_keys.get("language", [])
        annotation_meta = self.lerobot_modality_meta.annotation
        if not lang_keys or annotation_meta is None:
            return None

        language_key = lang_keys[0].replace("annotation.", "", 1)
        if language_key not in annotation_meta:
            return None
        original_key = annotation_meta[language_key].original_key or lang_keys[0]
        if original_key not in self.curr_traj_data.columns:
            return None
        return self._to_int_scalar(self.curr_traj_data[original_key].iloc[step_index])

    def _get_seg_for_step(self, step_index: int) -> list[dict] | None:
        if self.curr_traj_data is None:
            return None

        view_dict = {}
        for column in self.curr_traj_data.columns:
            if not str(column).startswith("segmentation."):
                continue

            seg_item = self.curr_traj_data[column].iloc[step_index]
            if seg_item is None:
                continue
            if isinstance(seg_item, (float, np.floating)) and np.isnan(seg_item):
                continue
            if isinstance(seg_item, str):
                try:
                    seg_item = json.loads(seg_item)
                except json.JSONDecodeError as e:
                    print(f"Warning: skip malformed segmentation JSON at {column}: {e}")
                    continue
            if not seg_item:
                continue

            view_name = str(column).replace("segmentation.", "", 1)
            view_dict[view_name] = self._enrich_seg_with_patches(seg_item)

        if not view_dict:
            return None
        return [view_dict]

    def _get_answers_for_step(self, step_index: int) -> dict | None:
        if self.answers.empty:
            return None

        task_index = self._get_task_index_for_step(step_index)
        if task_index is None or task_index not in self.answers.index:
            return None

        answer_row = self.answers.loc[task_index]
        if isinstance(answer_row, pd.DataFrame):
            answer_row = answer_row.iloc[0]
        return answer_row.to_dict()
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        step_row = self._get_step_row(base_index)
        sample = self._pack_sample(data, base_index=base_index, step_row=step_row)
        sample["__padt_meta__"] = {
            "dataset_name": self.dataset_name,
    ...
    def _get_step_row(self, base_index: int):
        if self.curr_traj_data is None:
            return None
        try:
            return self.curr_traj_data.iloc[int(base_index)]
        except Exception:
            return None

    def _maybe_enrich_padt_sample(self, sample: dict, step_row=None) -> dict:
        if getattr(self, "_padt_segmentation_adapter", None) is None:
            return sample
        try:
            return self._padt_segmentation_adapter.enrich_sample(sample, step_row)
        except Exception:
            if self.data_cfg is not None and bool(self.data_cfg.get("padt_task_meta_required", False)):
                raise
            return sample

    def _pack_sample(self, data: dict, base_index: int | None = None, step_row=None) -> dict:
        """Pack transformed modality data into training sample format."""
        prim_images = []
        wrist_views = []
        for video_key in self.modality_keys["video"]:
            image = data[video_key][0]
            image = Image.fromarray(image).resize(self._output_image_size)
        ...
        if base_index is not None and self._is_data_cfg_enabled("include_segmentation", False):
            seg = self._get_seg_for_step(base_index)
            if seg is not None:
                sample["seg"] = seg

        if base_index is not None and self._is_data_cfg_enabled("include_answers", False):
            answers = self._get_answers_for_step(base_index)
            if answers is not None:
                sample["answers"] = answers

        sample = self._maybe_enrich_padt_sample(sample, step_row=step_row)
        return sample
    ...
    def get_video_path(self, trajectory_id: int, key: str) -> Path:
        ...
        if self._lerobot_version == "v2.0":
            ...
            episode_meta = self.trajectory_ids_to_metadata[trajectory_id]
            # Use video-specific chunk/file indices (may differ from data chunk/file indices)
            video_chunk_indices = episode_meta.get("videos/chunk_indices", {})
            video_file_indices = episode_meta.get("videos/file_indices", {})
            vid_chunk = video_chunk_indices.get(original_key, episode_meta["data/chunk_index"])
            vid_file = video_file_indices.get(original_key, episode_meta["data/file_index"])
            video_filename = self.video_path_pattern.format(
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                step_row = dataset._get_step_row(step)
                sample = dataset._pack_sample(data, step_row=step_row)
