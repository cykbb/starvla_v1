...
# 
    def __init__(
        ...
        **kwargs,
        ...
        self._debug_logged_state = False

    def _log_state_debug_once(self, examples: List[dict], state: Optional[List[np.ndarray]]) -> None:
        if self._debug_logged_state:
            return
        if dist.is_initialized() and dist.get_rank() != 0:
            return

        first_example = examples[0]
        state_shape = tuple(np.asarray(first_example["state"]).shape) if state is not None else None
        action_shape = tuple(np.asarray(first_example["action"]).shape)
        logger.info(
            "Debug first training batch | keys=%s | has_state=%s | first_action_shape=%s | first_state_shape=%s",
    ...
    def forward(
        ...
        **kwargs,
        ...
        self._log_state_debug_once(examples, state)
    ...
    def predict_action( # TODO align  predict_action with forward, make api more flexible
        ...
        **kwargs: str,
        ...
        # Keep inference image handling aligned with training. The training loader
        # passes already-resized PIL images into `forward()` without consulting
        # `default_image_resolution`, so using that field only at inference creates
        # an avoidable train/eval image-scale mismatch.
